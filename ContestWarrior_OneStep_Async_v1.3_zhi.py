...
class OCR:
    ...
    async def aiohttp_post(self):
        """协程化，异步IO"""
        if not manul_headers:
            data = {'image': self.data_bs64_encode(), 'scene': self.sub_sce, 'parameters': {'vis_flag': False}}
        else:
            parameters_keys = '|'.join(manul_headers)
            data = {'image': self.data_bs64_encode(), 'scene': self.sub_sce, 'parameters': {'vis_flag': False, 'sdk': True, 'keys': parameters_keys}}
...
class Collector:
    ...
    def check_ocr_lab(self, lgot, ogot, headers_keys):
        for h_keys in headers_keys:
            if h_keys not in lgot:
                lgot[h_keys] = "None"
            if h_keys not in ogot:
                ogot[h_keys] = "None"
        return lgot, ogot

    def deal_final_info(self, lgot, ogot, keys_ocr, headers_keys):
        the_apple = OrderedDict()
        the_apple['文件名/准确率'] = self.file_name
        lgot, ogot = self.check_ocr_lab(lgot, ogot, headers_keys)
    ...
    async def processing_room(self, session_up):
        try:
            ocrs_got = await self.select_ocr_res(session_up)
